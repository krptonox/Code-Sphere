...
- **Fix applied:** Removed `next()` usage from async middleware to avoid runtime `next is not a function`.
...
April 3, 2026 - Updated middleware docs to match current implementation and documented runtime issues fixed.

## Recent Issues and Why Changes Were Made

1. **Issue:** `TypeError: next is not a function` from user model pre-save hook
   - **Why it happened:** async middleware used callback-style `next()`, which is invalid in this pattern.
   - **Change made:** switched to pure async pre-save hook and early return when password is unchanged.

2. **Issue impact:** user registration failed at `User.create(...)` before document save completed
   - **Why fix matters:** password hashing now runs reliably and registration can complete.
